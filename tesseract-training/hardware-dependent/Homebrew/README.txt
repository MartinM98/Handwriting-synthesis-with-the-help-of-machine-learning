...
- install_tesseract.sh : installs tesseract itself and training tools (should be run only once).
    Note that tesseract along with the training tools are installed from source.
- train-brew.sh : trains tesseract

In order to use the train-brew.sh script one should run it with the 
...
or relative) Here are a example:
    ./train-brew.sh /absoulte/path/to/the/directory/with/images
